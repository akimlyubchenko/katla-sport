...
export class HiveSectionListComponent implements OnInit {
  ...
  this.route.params.subscribe(p => {
    ...
    var section = this.hiveSections.find(h => h.id == hiveSectionId);
    this.hiveSectionService.setHiveSectionStatus(hiveSectionId, true).subscribe(c => section.isDeleted = true);
  }

   onUndelete(hiveSectionId: number) {
    var section = this.hiveSections.find(h => h.id == hiveSectionId);
    this.hiveSectionService.setHiveSectionStatus(hiveSectionId, false).subscribe(c => section.isDeleted = false);
  }
}
